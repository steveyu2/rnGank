...
class rnGank extends Component {
  ...
  componentDidMount() {
    const { fetchRandomData, fetchAppointTypeData } = this.props;
  ...
  handleUpdateLangugeIndex(selectedLangugeIndex) {
    ...
    this.props.setUserSetting({
      languge: this.selectedLanguge[selectedLangugeIndex]
    });
  }
  ...
  handleUpdateThemeIndex(selectedThemeIndex) {
    ...
    this.props.setUserSetting({
      themeName: this.selectedTheme[selectedThemeIndex]
    });
  }
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        />
        <Text style={{ fontSize: FONT_SIZE.HG, margin: 7 }}>
          huge size ABC abc {FONT_SIZE.HG}
        </Text>
        <Text style={{ fontSize: FONT_SIZE.LG, margin: 7 }}>
          large size ABC abc {FONT_SIZE.LG}
        </Text>
        <Text style={{ fontSize: FONT_SIZE.MD, margin: 7 }}>
          middle size ABC abc {FONT_SIZE.MD}
        </Text>
        <Text style={{ fontSize: FONT_SIZE.NM, margin: 7 }}>
          normal size ABC abc {FONT_SIZE.NM}
        </Text>
        <Text style={{ fontSize: FONT_SIZE.SM, margin: 7 }}>
          small size ABC abc {FONT_SIZE.SM}
        </Text>
        <Text style={{ fontSize: FONT_SIZE.XS, margin: 7 }}>
          xsmall size ABC abc {FONT_SIZE.XS}
        </Text>

        <Text style={[{ fontSize: FONT_SIZE.MD, margin: 7 }, theme.test]}>
          {i18n.randomRecommendation}
        </Text>
        <Text style={[{ fontSize: FONT_SIZE.MD, margin: 7 }, theme.test]}>
          languge: {languge}
        </Text>
